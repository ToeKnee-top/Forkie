...
async function executeTurn(
  ...
  // The sandbox: creating this object is free — the backing sandbox (E2B) only
  // materializes when a tool first touches it. It is PER-THREAD and PERSISTENT.
  // Provider selection: SSH_SANDBOX_HOST => run code over SSH on a box you own;
  // else E2B_API_KEY => the E2B-backed sandbox (paused/remembered per thread);
  // else run locally in this container — no E2B account or card verification
  // needed, and files persist naturally on the host filesystem.
