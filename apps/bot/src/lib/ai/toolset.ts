...
import { devlogTool } from './tools/devlog';
  ...
  const deferred: Record<string, { summary: string; tool: Tool }> = {
    ...
    ...(env.OPENWEATHER_API_KEY
      ? {
          weather: {
            summary:
              'check current weather (OpenWeather) for a city anywhere in the world',
            tool: weatherTool(),
          },
        }
      : {}),
    devlog: {
      summary:
        "fetch and summarize a Stardance devlog (stardance.hackclub.com) link into a ready-to-post 'Devlog Boosted' card",
      tool: devlogTool(),
    },
    ...(env.GH_TOKEN
      ? {
