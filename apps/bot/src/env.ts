...
export const env = createEnv({
  ...
  server: {
    ...
    // box you own; else E2B_API_KEY => E2B-backed sandbox; else run locally in
    // this container (no E2B account/card needed). SANDBOX_WORKDIR is the base
    // directory for the local/ssh sandbox.
    ...
    GH_TOKEN: z.string().min(1).optional(),    // The GitHub account that token belongs to — kyto's own identity there. Used
    ...
    // OpenWeatherMap key for the `weather` tool (port of QuackX's
    // /quackx-weather). Tool is registered only when set. Free key at
