...
function privacyBlocks(privacy: ErasePreview | undefined): SlackBlock[] {
  ...
  const blocks: SlackBlock[] = [
    ...
    {
      text: mrkdwn(
        `*Your data*\nForkie stores two things derived from your conversations: memories it saved, and a cache of its own reasoning from your last few turns (kept about 30 days). It never stores a copy of your messages — it reads the Slack thread each turn. Currently: ${counts}.`
      ),
      type: 'section',
    },
    {
